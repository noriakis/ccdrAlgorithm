ccdr.gridR <- function(X,                    # data matrix
                       betas,                # OPTIONAL beta matrix / DEFAULT = zeroes
                       lambdas,           # OPTIONAL lambda grid / DEFAULT = derived from data
                       nlam = 50,    	    # OPTIONAL number of lambdas / DEFAULT = 50
                       gamma = 2.0,       # OPTIONAL gamma parameter for MCP / DEFAULT = 2.0
                       eps = 1e-4,	    # OPTIONAL error threshold / DEFAULT = 1e-4
                       rlam = 1e-4,       # OPTIONAL ratio between min.lam : max.lam / DEFAULT = 1e-4
                       maxIters,	        # OPTIONAL maximum number of iterations / DEFAULT = 2 * max(10, sqrt(pp))
                       alpha = 2.0,       # OPTIONAL stopping criterion / DEFAULT = 2.0
                       verbose = FALSE,   # OPTIONAL flag for printing out progress / DEFAULT = FALSE
                       as.mat = TRUE      # OPTIONAL convert results to R Matrix class or not?
){
    ...
    ccdr.out <- list()
    for(i in 1:nlam){
        cat("Working on lambda = ", lambdas[i], " [", i, "/", nlam, "]\n", sep = "")
...
ccdr.singleR <- function(c,                    # correlation vector
                         pp, nn,
                         betas,    	        # OPTIONAL beta matrix / DEFAULT = zeroes
                         lambda,
                         gamma = 2.0,       # OPTIONAL gamma parameter for MCP / DEFAULT = 2.0
    ...
    ccdr.out <- singleCCDr(c, 
                           ...
                           verbose = verbose)
    t2.ccdr <- proc.time()[3]
    cat("C++ connection closed. Total time in C++: ", t2.ccdr-t1.ccdr, "\n")
