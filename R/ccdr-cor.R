...
cor_vector_ivn <- function(X, ivn){
    ...
    pp <- ncol(X)
    ivnj <- as.integer(c(sort(unique(unlist(ivn))), pp + 1))
    # all the j's that has interventions
    # including pp+1 for observational rows (compatible with purely observational data)
    # necessary?? if we mostly do balanced designs where all nodes get intervention
