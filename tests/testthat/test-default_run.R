...
test_that("ccdr.run with intervention, case 1:", {
    ...
    ivnvector <- q1[as.integer(as.vector(sapply(1:pp, rep, nn)))]
    data <- sparsebnUtils::sparsebnData(X1, type = "c", ivn = as.list(ivnvector))

    final <- ccdr.run(data = data, lambdas.length = 10) ## use most of the default settings
    expect_is(final, "list")
